...
```
  ...
  └──────────┬───────────┘     └──────────┬────────────┘     └────────────┬─────────────┘
     ...
     ▼                  ▼                 ▼                                ▼
┌──────────────┐  ┌─────────────────┐  ┌─────────────────────────┐  ┌───────────────────┐
│ Per-Report   │  │      PAX        │  │  Super Usage Analysis   │  │  GitHub Copilot   │
│  Scripts     │  │  (standalone)   │  │  Superuser Impact       │  │  Impact           │
│              │  │                 │  │  CustomizeCopilot       │  │                   │
│ Bundled with │  │ Enterprise-grade│  └─────────────────────────┘  └───────────────────┘
│ each report. │  │ No row limits.  │
│ Feeds that   │  │ Schedulable.    │
│ one template.│  │                 │
│              │  │ → Data Lake     │
│ Includes     │  │ → Warehouse     │
│ Azure Auto-  │  │ → Fabric/Synapse│
│ mation runb- │  │ → SIEM          │
│ ooks for     │  │ → Any tool      │
│ scheduling.  │  │ → PBI templates │
└──────┬───────┘  └────────┬────────┘
       │                   │
       └─────────┬─────────┘
                 │  (CSV / Excel output)
                 ▼
  ┌──────────────────────────────────────────────────────────┐
  │              Purview-Based PBI Templates                 │
  ├──────────────────┬──────────────────┬────────────────────┤
  │  AI-in-One       │  Copilot Chat &  │  M365 Copilot      │
  │  Dashboard       │  Agent Intel.    │  Readiness         │
  └──────────────────┴──────────────────┴────────────────────┘
```
...
- ⭐ **[Star this hub](https://github.com/microsoft/Analytics-Hub/stargazers)** — bookmarks it and helps others discover it
- 👀 **[Watch this hub](https://github.com/microsoft/Analytics-Hub/subscription)** — get notified when new repos are added
